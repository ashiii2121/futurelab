...
const Checkups = () => {
    ...
    const fetchTabs = async () => {
      try {
        const response = await fetch(
          `${baseUrl}/api/v1/category/lessPrice/get`
        );
        ...
        if (data.success && data.data) {
          ...
          const initialTab = tabFromUrl || data.data[0]?.name || "";
  ...
  const fetchTabContent = async (tabName) => {
    try {
      setLoading(true);
      const response = await fetch(
        `${baseUrl}/api/v1/tests/category/${encodeURIComponent(tabName)}`
      );
      const data = await response.json();
  ...
  return (
    <div className="checkups-page">
      <div className="container py-5">
        <h2 className="mb-4">Health Checkup Packages</h2>
                  ...
                  <div className="card test-card h-100">
                    <div className="card-body">
                      ...
                      <div className="mt-3 d-flex flex-wrap gap-2">
                        <button
                          className="btn btn-know-more btn-know-more"
